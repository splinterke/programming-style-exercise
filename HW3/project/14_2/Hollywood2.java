...
public class Hollywood2 {
	...
	public static void main(String[] args){
		...
		ZCounter zc = new ZCounter(fw,rf);
		fw.run(args[0]);
...
class ZCounter extends dosomething {
	FrameWork _f_w = new FrameWork();
    ReadFile _r_f = new ReadFile(_f_w);

	ZCounter(FrameWork fw, ReadFile rf) {
		fw.register_for_print(this);
		_r_f = rf;
	}
	...
	public void act(String information){
		...
		for(int i=0;i<token.length;i++) {
			boolean has_z = false;
			String word_now = token[i];
			for(int j=0;j<word_now.length();j++){
				if(word_now.charAt(j)=='z'){
					has_z = true;
				}
			}
			if(has_z){
		...
		System.out.println("Number of words with 'z' is : " + z_num);
	}
}
